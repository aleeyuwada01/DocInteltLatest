...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // Allow GET for share mode, POST for everything else
  const isShareMode = req.query.mode === 'share' || req.body?.mode === 'share';
  if (req.method !== 'POST' && !isShareMode) return res.status(405).json({ error: 'Method not allowed' });

  // ── Share Mode (public – no user auth needed) ──────────────────────────────
  // Merged from api/share/[token].ts - runs before auth check
  if (isShareMode) {
    const shareToken = (req.query.token as string) || req.body?.token;
    ...
    try {
      ...
      if (linkError || !link) return res.status(404).json({ error: 'Share link not found' });
      ...
      if (file.trashed_at) return res.status(410).json({ error: 'This file has been deleted.' });

      const { data: signedData, error: signedError } = await supabaseAdmin.storage
        .from('uploads')
        .createSignedUrl(file.storage_path, 7200);

      if (signedError || !signedData?.signedUrl) return res.status(500).json({ error: 'Could not generate file access URL' });

      supabaseAdmin.rpc('increment_share_access', { share_token: shareToken }).then();

      return res.status(200).json({
        file: { id: file.id, original_name: file.original_name, size: file.size, mime_type: file.mime_type },
  ...
  // Below this point: authenticated routes only
